...
class AjaxController extends AdminController
{
    ...
    {
        Cache::flush();
        return $this->success('キャッシュをクリアしました');
    }
    ...
    public function upload(): View|JsonResponse
    {
        ...
        $validator = Validator::make($data, $rules, [
            'upload_type' => '指定されたアップロードタイプが正しくありません',
            'file'        => 'ファイルは必須です',
        ]);
        if ($validator->fails()) {
        ...
        if (!in_array($file->extension(), explode(',', $uploadConfig['upload_allow_ext']))) {
            return $this->error('アップロードファイルの種類が許可範囲外です');
        }
        if ($file->getSize() > $uploadConfig['upload_allow_size']) {
            return $this->error('ファイルサイズが設定値を超えています');
        }
        ...
        if ($code == 0) {
            ...
            return $type == 'editor' ? json(
                [
                    'error'    => ['message' => 'アップロードしました', 'number' => 201,],
                    'fileName' => '',
                    'uploaded' => 1,
                    'url'      => $upload['data']['url'] ?? '',
                ]
            ) : $this->success('アップロードしました', $upload['data'] ?? '');
        }
    }
    ...
    public function uploadUEditor(): JsonResponse
    {
        ...
        switch ($action) {
            ...
            case 'attachment':
            case 'video':
                if ($this->isDemo) return json(['state' => 'デモ環境では変更できません']);
                try {
                    $upload = UploadService::instance()->setConfig($uploadConfig)->$upload_type($file);
                    $code   = $upload['code'] ?? 0;
                    if ($code == 0) {
                        return json(['state' => $upload['data'] ?? 'アップロードエラー情報']);
                    }else {
