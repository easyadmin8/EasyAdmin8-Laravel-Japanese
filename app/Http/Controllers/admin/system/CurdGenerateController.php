...
#[ControllerAnnotation(title: 'CURDビジュアル管理')]
    ...
    public function save(): Response|JsonResponse|View
    {
        ...
        switch ($type) {
            case "search":
                ...
                if (empty($tb_name)) return $this->error('パラメーターエラー');
                try {
                    ...
                    return $this->success('検索しました', compact('data', 'list'));
            ...
            case "add":
                ...
                try {
                    ...
                    if (empty($fileList)) return $this->error('ここには何もありません');
                    $result = $build->create();
                    $_file  = $result[0] ?? '';
                    $link   = '';
                    if (!empty($_file)) {
                    ...
                    return $this->success('生成しました', compact('result', 'link'));
            ...
            case "delete":
                ...
                try {
                    ...
                    if (empty($fileList)) return $this->error('ここには何もありません');
                    $result = $build->delete();
                    return $this->success('自動生成CURDファイルを削除しました', compact('result'));
                }catch (FileException $exception) {
                    return json(['code' => -1, 'msg' => $exception->getMessage(), '__token__' => csrf_token()]);
                }
                break;
            case 'console':
                ...
                if (empty($command)) $this->error('コマンドを入力してください');
                ...
                if (empty($output)) $this->error('設定エラー');
                return $this->success();
                break;
            default:
                return $this->error('パラメーターエラー');
                break;
        }
    }
