...
#[ControllerAnnotation(title: 'システムノード管理')]
class NodeController extends AdminController
{
    ...
    #[NodeAnnotation(title: 'システムノード更新', auth: true)]
    public function refreshNode(): JsonResponse
    {
        $force = request()->input('force');
        if (!request()->ajax()) return $this->error();
        $nodeList = (new NodeService())->getNodeList();
        if (empty($nodeList)) return $this->error('更新するシステムノードがありません');
        ...
        try {
            ...
            return $this->error('ノード更新失敗:' . $e->getMessage());
        }
        return $this->success('ノードを更新しました');
    }

    #[NodeAnnotation(title: '無効なノードをクリア', auth: true)]
