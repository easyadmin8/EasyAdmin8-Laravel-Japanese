...
if (!function_exists('parse_name')) {
    /**
     * 文字列命名スタイル変換
     * type 0 Java形式をC形式に変換 1 C形式をJava形式に変換
     * @param string $name 文字列
     * @param int $type 変換タイプ
     * @param bool $ucfirst 先頭文字を大文字にするか（キャメルケース）
     * @return string
     */
 ...
 * @return string
 */
function editor_textarea(?string $detail, string $name = 'desc', string $placeholder = '入力してください'): string
{
...
/**
 * @desc Excelエクスポート
 * @tip パフォーマンスを重視する場合は xlsWriter https://xlswriter-docs.viest.me/zh-cn を使用してください
 * @param array $header
 * @param array $list
...
function exportExcel(array $header = [], array $list = [], string $fileName = ''): void
{
    ...
    if (empty($header) || empty($list)) throw new \Exception('エクスポートデータは必須です');
