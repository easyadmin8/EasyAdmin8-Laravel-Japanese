        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.code === 1 && res.data && res.data.directories) {
                    ...
                    $('#logTree').html('<div style="text-align:center;color:#999;padding:50px;"><i class="layui-icon layui-icon-face-cry" style="font-size:30px;"></i><p style="margin-top:10px;">' + (res.msg || '読み込み失敗') + '</p></div>');
                }
            },
            error: function (error) {
                $('#logTree').html('<div style="text-align:center;color:#999;padding:50px;"><i class="layui-icon layui-icon-face-cry" style="font-size:30px;"></i><p style="margin-top:10px;">リクエスト失敗</p></div>');
            }
        });
    }
    ...
    return {
        index: function () {
            ...
            $('#analyzeBtn').on('click', function () {
                if (selectedFiles.length === 0) return layer.msg('ログファイルを選択してください', {icon: 0});
                $('#loadingMask').hide();
                ...
                ea.request.get({
                    ...
                    showResult('<div style="text-align:center;padding:150px 0;"><i class="layui-icon layui-icon-loading-1 layui-anim layui-anim-rotate layui-anim-loop" style="font-size:48px;"></i><p style="margin-top:20px;">分析中...</p></div>');
                    ea.request.post({
                        ...
                        showResult('<div class="markdown-body"><div style="background:#e8f4fd;padding:10px;border-radius:8px;margin-bottom:15px;font-size:1rem;color:#666;">読み込み完了 ' + res.data.metadata.total_files + ' ファイル、合計 ' + res.data.metadata.total_lines + ' 行</div>' + marked.parse(res2.data.analysis) + '</div>');
                        layer.msg('分析完了', {icon: 1});
                    }, function (err2) {
                        $('#loadingMask').hide();
                        $('#analyzeBtn').prop('disabled', false);
        ...
        realtime: function () {
            ...
            ea.request.get({url: ea.url('system/logAnalyzer/realtime'), data: {type: 'summary'}}, function (res) {
                show(res.data.summary || res.data.analysis || 'データがありません');
            });

            $('#refreshBtn').click(function () {
                let btn = $(this).prop('disabled', true).html('<i class="layui-icon layui-icon-loading layui-anim layui-anim-rotate"></i> 更新中');
                show('更新中...');
                ea.request.get({url: ea.url('system/logAnalyzer/realtime'), data: {type: 'summary'}}, function (res) {
                    btn.prop('disabled', false).html('<i class="layui-icon layui-icon-refresh"></i> 更新');
                show(res.data.summary || res.data.analysis || 'データがありません');
                });
            });
        },
