...
define(['jquery'], function (colorMode) {
    ...
    var colorMode = {
         ...
         * @prop {string} [selector="html"] - 対象要素の CSS セレクタ
         * @prop {string} [attribute="class"] - 対象要素の HTML 属性
         * @prop {string} [initialValue='auto'] - 初期カラーモード
         * @prop {Object.<string, string>} [modes]- カラーモード。value は HTML 属性に追加される値
         * @prop {(mode: string, defaultHandler: (window?: Window) => void) => void} [onChanged] - 更新用のカスタムハンドラ。指定時はデフォルト動作が上書きされます。
         * @prop {Storage} [storage=localStorage] - localStorage/sessionStorage への永続化キー。`null` を渡すと永続化を無効化
         * @prop {string | null} [storageKey='color-scheme'] - 永続化に使用するキー
         * @prop {boolean} [disableTransition=true] - 切り替え時のトランジションを無効化 {@link https://paco.me/writing/disable-theme-transitions}
         *
         */
