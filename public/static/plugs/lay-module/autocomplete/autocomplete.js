    ...
    system.init = function (e, c) {
        ...
        _elems.each(function (_i, _e) {
            ...
            try {
                ...
                return hint.error("autocomplete要素属性lay-dataの設定に構文エラーがあります：" + _lay_data)
            }
            var _config = $.extend({ elem: this }, system.config, c, _lay_data);
            _config.url == undefined && (_config.data == undefined || _config.length === 0) && hint.error("autocomplete設定エラー：データ取得方法が不足しています");
